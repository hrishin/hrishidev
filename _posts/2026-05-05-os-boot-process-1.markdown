---
layout: post
title:  "From Power Button to Shell Prompt, Part 1: The Linux Boot Process From Firmware to Login"
date:   2026-05-05 06:00:00 +0000
categories: [Linux, OS, Systems]
description: "A stage-by-stage walkthrough of the Linux boot process on x86-64: the reset vector, BIOS/UEFI, POST, GRUB2 and Secure Boot, kernel init, initramfs, systemd, and PAM-based login. Part 1 of a 3-part series that continues with PXE network boot and Metal3 bare-metal provisioning."
...
This is a 3-part series that follows the same seven stages from a laptop to a rack of bare-metal servers:

- **Part 1 (this post): From Firmware to Login**, a disk boot end to end: the reset vector, BIOS/UEFI, POST, shim
  and GRUB2, kernel init, initramfs, systemd, and PAM.
- **[Part 2: PXE Network Boot with Metal³ Bare Metal Operator and Ironic](https://hrishi.dev/linux/os/systems/2026/09/18/os-boot-process-2.html)**, the same stages on a server with
  no operating system on its disk. Stage 3 becomes a DHCP, TFTP, and HTTP conversation, a DHCP relay carries it
  across VLANs, and the Bare Metal Operator drives Ironic to boot a RAM-only agent that writes the OS image.
- **[Part 3: Automated Host Provisioning with Metal³, CAPM3, and Cluster API](https://hrishi.dev/linux/os/systems/2026/09/19/os-boot-process-3.html)**, Cluster API on top of that
  machinery, so one `Cluster` manifest claims, images, and joins a pool of inventoried hosts: the full call
  sequence, the object model, and the two state machines.
...
machine is actually doing before that shell prompt appears. Every stage is a narrow handoff: firmware finds a
bootloader, the bootloader finds a kernel, the kernel finds a root filesystem, and PID 1 finds the services that
finally hand you a prompt.
...
Everything in this part assumes a disk with an operating system on it. A server fresh from the rack has none.
**[Part 2: PXE Network Boot with Metal³ Bare Metal Operator and Ironic](https://hrishi.dev/linux/os/systems/2026/09/18/os-boot-process-2.html)** takes the same seven stages onto
that server: Stage 3 becomes `PXE ROM → DHCP → TFTP → iPXE → HTTP`, the DHCP relay carries it across VLANs, and
the first Linux to boot is a RAM-only agent that installs the real one.
